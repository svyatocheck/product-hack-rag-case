...
| Answer Correctness  | Faithfulness  | Context Recall  |  Context Utilization |
|---|---|---|---|
| 0.96 | 0.72   | 0.78  | 0.94   |

**Примечание:**
Плохие показатели для контекста получились из-за ограничения на число чанков (2, когда наше решение работает с 10+) при оценке. Пришлось вручную урезать список.
...
- Сервисный аккаунт: сервисный аккаунт, который позволит обращаться к другим сервисам из проекта DataSphere. Важно, что сервисному аккаунту должны быть назначены роли `ai.languageModels.user` для доступа к модели YandexGPT, `vpc.user`.
- Подсеть: укажите подсеть, в которой находятся сервис YandexGPT и другие.
...
- перейдите в директорию `docker/` проекта
- установите ключи и прочие параметры в `keys.py`, удобным для вас способом (извлечение из .env, в строку и проч.)
- создайте контейнеры по команде `docker compose -f docker-compose.yml up --build` (из `docker/` директории), перед этим запустив [Docker Сервисы](https://docs.docker.com/desktop/install/windows-install/)
- дождитесь окончания сборки образа - процесс занимает много времени из-за тяжелых зависимостей.

**После сборки вам станут доступны 2 способа общения с чат-ботом:**

- Пользовательский интерфейс чат-бота, написанный на Streamlit. Интерфейс станет доступен по [ссылке](http://localhost:8501). (Первая вкладка меню не имеет глубокой проработки - сразу переходите на Чат).
